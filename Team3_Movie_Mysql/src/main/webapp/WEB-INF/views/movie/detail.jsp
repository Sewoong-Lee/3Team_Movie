...
$(function() {
	//youtube api 우선 youtube api 제재로 인해 주석처리
  	function youtube() {
	 	...
	 	console.log(movieCode+ movieName+youtube_link);
		if(youtube_link !=''){ 
...
function rederReplyDetail(data) {
	...
	$('#Ticketing').click(function(e) {
		...
		if(${sessionScope.user_id==null}){
			alert('로그인 후 이용 해주세요')
			return location.href="${path}/user/login";
		}
...
<%-- 			<td>${reList.MR_NUM}</td> --%>
			...
			<td class="cStar">	
				<c:choose>
					...
					<c:when test="${reList.STAR_RATING>=2.0}"><img alt="Rstar" src="https://image.aladin.co.kr/img/shop/2012/icon_star4.png"></c:when>
					<c:when test="${reList.STAR_RATING>=1.5}"><img alt="Rstar" src="https://image.aladin.co.kr/img/shop/2012/icon_star3.png"></c:when>
					<c:when test="${reList.STAR_RATING>=1.0}"><img alt="Rstar" src="https://image.aladin.co.kr/img/shop/2012/icon_star2.png"></c:when>
					<c:when test="${reList.STAR_RATING>=0.5}"><img alt="Rstar" src="https://image.aladin.co.kr/img/shop/2012/icon_star1.png"></c:when>
